...
import static java.lang.System.err;//fig.basic.LogInfo.err;
import static java.lang.System.out; //fig.basic.LogInfo.stdout;
...
class OptInfo {
  ...
  public String fullName() { return formatPrefix(group)+name; }
  
  public static String formatPrefix(String group) { return group == null || group.equals("") ? "" : group + "."; }
  ...
  public Object getValue() {
    try {
      ...
      err.println("Can't access method: " + e);
      return null;
    } catch(IllegalAccessException e) {
      err.println("Can't access field: " + e);
      return null;
    }
  }
  ...
  public String getValueString() {
    ...
    if(o instanceof Pair)
      return ((Pair)o).getLeft() + "," + ((Pair)o).getRight();
  ...
  public void print() { out.println("  " + toString()); }
  ...
  private static boolean checkNumArgs(int want, int have, String fullName) {
    if(have != want) {
      err.printf(want + " arguments required for " + fullName + ", but got " + have + "\n");
      return false;
    }
    return true;
  }
...
//    }
    ...
    if(type instanceof Class && ((Class)type).isEnum()) {
      ...
      if(x == null) {
        err.println("Invalid enum: '" + firstArg + "'; valid choices: " + getEnumStr((Class)type));
        return errorValue;
      }
      return x;
    }
    ...
    if(typeIsArray(type)) {
      ...
      int i = 0;
      for(String a : l) {
        Object o = interpretValue(childType, Lists.newArrayList(a), fullName);
    ...
    if(type instanceof ParameterizedType) {
      ...
      if(type.equals(Pair.class)) { // Delimited by comma
        ...
        if (firstArg.contains(",")) delim = ",";
          ...
          err.println("Invalid pair: '" + firstArg + "' (delimiter is either ',' or ':')");
          return errorValue;
        }
        String[] tokens = firstArg.split(delim, 2);
        Object o1 = interpretValue(childTypes[0], Lists.newArrayList(tokens[0]), fullName);
        if(o1 == errorValue) return errorValue;
        Object o2 = interpretValue(childTypes[1], Lists.newArrayList(tokens[1]), fullName);
        ...
        // Put the elements in the array
        for(String a : l) {
          Object o = interpretValue(childTypes[0], Lists.newArrayList(a), fullName);
          if(o == errorValue) return errorValue;
          x.add(o);
        }
        return x;
    ...
    if(type instanceof Class) {
      try {
        ...
        err.println("Failed to construct " + type + ": " + e);
        e.printStackTrace();
        return errorValue;
      }
    }

    err.println("Can't handle weird field type: " + type);
    return errorValue;
  }
  ...
  public boolean set(List<String> l, boolean append) {
    try {
      ...
      err.println("Can't set method: " + e);
      return false;
    } catch(IllegalAccessException e) {
      err.println("Can't set field: " + e);
      return false;
    }
...
public class OptionsParser {
  ...
  public OptionsParser register(Object o) { return register("", o); }
  ...
  public OptionsParser register(String group, Object o) {
    ...
    for(Field field : classOf(o).getFields()) {
      ...
      try {
        register(OptInfo.formatPrefix(group)+ann.name(), field.get(o));
      } catch(IllegalAccessException e) {
    ...
    for(Method method : classOf(o).getMethods()) {
      ...
      if(ann == null) continue;
      try {
        register(OptInfo.formatPrefix(group)+ann.name(), method.invoke(o));
      } catch(InvocationTargetException e) {
  ...
  private List<OptInfo> matchOpt(ArrayList<OptInfo> options, String s,
    ...
    for(OptInfo opt : options) {
      ...
      if(!mustMatchFullName 
          && t.contains(".")) // we don't want to match top level elements twice
      {
    ...
    if(completeMatches.size()+partialMatches.size() == 0) {
      if(!ignoreUnknownOpts)
        err.println("Unknown option: '" + s + "'; -help for usage");
    ...
    if(allowMultipleMatches)
      ...
      if(completeMatches.size() == 1)
        return Lists.newArrayList(completeMatches.get(0));
      if(completeMatches.size() == 0 && partialMatches.size() == 1)
        return Lists.newArrayList(partialMatches.get(0));

      err.println("Ambiguous option: '" + s + "'; possible matches:");
  ...
  public boolean parse(String[] args) {
    ...
    for(int i = 0; i < args.length;) {
      if(args[i].equals("-help")) { // Get usage help
        ...
        err.println("Argument not part of an option: " + args[i]);
    ...
    if(!relaxRequired) {
      ...
      if(missingOptMsgs.size() > 0) {
        err.println("Missing required option(s):");
  ...
  private OptInfo findOptInfo(List<OptInfo> optInfos, String name, String group) {
    ...
    name = OptInfo.formatPrefix(group) + name;
    for(OptInfo info : optInfos)
      if(info.fullName().equals(name)) return info;
    return null;
  }
